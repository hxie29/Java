/*(Create a file for a graph) Modify Listing 29.3, TestWeightedGraph.java, to
...
public class E2910 {
    public static void main(String[] args) {
        ...
        try (PrintWriter output = new PrintWriter("E2910_result.txt")) {
            output.print(vertices.length);
            int count = -1;
            for (int[] edge : edges) {
                if (edge[1] > edge[0]) {
                    if (edge[0] == count)
                        output.print(" | " + edge[0] + ", " + edge[1] + ", " + edge[2]);
                    else {
                        count++;
