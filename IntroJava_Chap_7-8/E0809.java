...
public class E0809 {
    ...
    public static void printBoard(String[][] boardStr) {
        ...
        for (int i =0; i < boardStr.length; i++) {
            System.out.printf("| %1S | %1S | %1S |\n", boardStr[i][0], boardStr[i][1], boardStr[i][2]);
            System.out.println("-------------");
        }
    }
    ...
    public static int verifyGame(int[][] board) {
        int status = 0;
        if (diagonal(1, board) || horizontal(1, board) || vertical(1, board)) status = 1;
        else if (diagonal(2, board) || horizontal(2, board) || vertical(2, board)) status = 2;
        else if (!fullBoard(board)) status = 0;
        else status = 3;
        return status;
    }
    ...
    public static boolean horizontal(int n, int[][]board) {
        for (int i = 0; i < board.length; i++) {
            if (board[i][0] == n && board[i][1] == n && board[i][2] == n) return true;
        }
        return false;
    }
    ...
    public static boolean diagonal(int n, int[][]board) {
        ...
        for (int i = 0; i < board.length; i++) {
            if (board[i][i] != n) majorDiagonal = false;
        }
        for (int i = 0; i < board.length; i++) {
            if (board[i][2-i] != n) minorDiagonal = false;
        }
